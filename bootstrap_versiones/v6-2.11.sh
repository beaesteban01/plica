...
#
# Install Spark 2.4.4 --> 2.4.7 (Scala 2.11)
# 
# Run with: spark-submit --packages org.apache.spark:spark-sql-kafka-0-10_2.11:2.4.7 StructuredStreamingAndElasticPrueba.py

echo "" | tee -a $LOG_FILE
echo "Downloading and installing Spark 2.4.7 ..." | tee -a $LOG_FILE
curl -Lko /tmp/spark-2.4.7-bin-without-hadoop.tgz https://archive.apache.org/dist/spark/spark-2.4.7/spark-2.4.7-bin-without-hadoop.tgz

mkdir -p /home/vagrant/spark
cd /home/vagrant
tar -xvf /tmp/spark-2.4.7-bin-without-hadoop.tgz -C spark --strip-components=1
...
echo "spark.jars /home/vagrant/PLICA/lib/commons-httpclient-3.1.0.jar" | sudo tee -a /home/vagrant/spark/conf/spark-defaults.conf

#
# ACTUALIZADO 2.1.1 --> 2.4.1 (scala 2.11)
...
curl -Lko /tmp/kafka_2.11-2.4.1.tgz http://apache.uvigo.es/kafka/2.4.1/kafka_2.11-2.4.1.tgz
