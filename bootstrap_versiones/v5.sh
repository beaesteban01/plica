...
#
# Install Hadoop 3.0.1 --> 3.3.0
#
curl -Lko /tmp/hadoop-3.3.0.tar.gz http://apache.uvigo.es/hadoop/common/hadoop-3.3.0/hadoop-3.3.0.tar.gz
mkdir -p /home/vagrant/hadoop
cd /home/vagrant/
tar -xvf /tmp/hadoop-3.3.0.tar.gz -C hadoop --strip-components=1
...
#
# Install Spark 2.4.4 --> 3.0.1 (Scala 2.12) (for hadoop 3.2 and later)
# 
...
curl -Lko /tmp/spark-3.0.1-bin-without-hadoop.tgz http://apache.uvigo.es/spark/spark-3.0.1/spark-3.0.1-bin-hadoop3.2.tgz
...
#
# ACTUALIZADO 2.1.1 --> 2.6.0 (scala 2.12)

# Kafka install and setup
#
echo "" | tee -a $LOG_FILE
echo "" | tee -a $LOG_FILE
echo "Downloading and installing Kafka version 2.6.0 for Scala 2.12 ..." | tee -a $LOG_FILE
#curl -Lko /tmp/kafka_2.11-2.1.1.tgz https://www-us.apache.org/dist/kafka/2.1.1/kafka_2.11-2.1.1.tgz
curl -Lko /tmp/kafka_2.12-2.6.0.tgz https://apache.brunneis.com/kafka/2.6.0/kafka_2.12-2.6.0.tgz

mkdir -p /home/vagrant/kafka
cd /home/vagrant/
tar -xvzf /tmp/kafka_2.12-2.6.0.tgz -C kafka --strip-components=1 && rm -f /tmp/kafka_2.12-2.6.0.tgz
...
rm kibana-7.9.1-linux-x86_64.tar.gz
