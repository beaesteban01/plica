...
curl -Lko /home/vagrant/PLICA/lib/commons-httpclient-3.1.jar http://central.maven.org/maven2/commons-httpclient/commons-httpclient/3.1/commons-httpclient-3.1.jar


#
# Install Hadoop 3.0.1 --> 3.1.4
#
curl -Lko /tmp/hadoop-3.1.4.tar.gz https://archive.apache.org/dist/hadoop/common/hadoop-3.1.4/hadoop-3.1.4.tar.gz
...
#
# Install Spark 2.4.4 --> 3.0.1 (Scala 2.12)
#
# Run with: spark-submit --packages org.apache.spark:spark-sql-kafka-0-10_2.12:3.0.1 StructuredStreamingAndElasticPrueba.py

echo "" | tee -a $LOG_FILE
echo "Downloading and installing Spark 3.0.1 ..." | tee -a $LOG_FILE
...
curl -Lko /tmp/spark-3.0.1-bin-without-hadoop.tgz https://apache.brunneis.com/spark/spark-3.0.1/spark-3.0.1-bin-hadoop2.7.tgz

mkdir -p /home/vagrant/spark
cd /home/vagrant
tar -xvf /tmp/spark-3.0.1-bin-without-hadoop.tgz -C spark --strip-components=1
...
#
echo "curl -sLko /tmp/elasticsearch-7.9.1.tar.gz https://artifacts.elastic.co/downloads/elasticsearch/elasticsearch-7.9.1-linux-x86_64.tar.gz"
curl -sLko /tmp/elasticsearch-7.9.1.tar.gz https://artifacts.elastic.co/downloads/elasticsearch/elasticsearch-7.9.1-linux-x86_64.tar.gz
mkdir /home/vagrant/elasticsearch
cd /home/vagrant
tar -xvzf /tmp/elasticsearch-7.9.1.tar.gz -C elasticsearch --strip-components=1
...
# Install Elasticsearch for Hadoop 6.1.2 --> 7.9.1
echo "curl -sLko /tmp/elasticsearch-hadoop-7.9.1.zip https://artifacts.elastic.co/downloads/elasticsearch-hadoop/elasticsearch-hadoop-7.9.1.zip"
curl -sLko /tmp/elasticsearch-hadoop-7.9.1.zip https://artifacts.elastic.co/downloads/elasticsearch-hadoop/elasticsearch-hadoop-7.9.1.zip

unzip /tmp/elasticsearch-hadoop-7.9.1.zip
mv /home/vagrant/elasticsearch-hadoop-7.9.1 /home/vagrant/elasticsearch-hadoop
cp /home/vagrant/elasticsearch-hadoop/dist/elasticsearch-hadoop-7.9.1.jar /home/vagrant/PLICA/lib/
cp /home/vagrant/elasticsearch-hadoop/dist/elasticsearch-spark-20_2.11-7.9.1.jar /home/vagrant/PLICA/lib/
echo "spark.speculation false" | sudo tee -a /home/vagrant/spark/conf/spark-defaults.conf
rm -f /tmp/elasticsearch-hadoop-7.9.1.zip
...
#OJO CAMBIAR AQUI TAMBIEN!
echo "spark.jars /home/vagrant/PLICA/lib/mongo-hadoop-spark-2.0.2.jar,/home/vagrant/PLICA/lib/mongo-java-driver-3.6.1.jar,/home/vagrant/PLICA/lib/mongo-hadoop-2.0.2.jar,/home/vagrant/PLICA/lib/elasticsearch-spark-20_2.11-7.9.1.jar,/home/vagrant/PLICA/lib/snappy-java-1.1.7.1.jar,/home/vagrant/PLICA/lib/lzo-hadoop-1.0.5.jar,/home/vagrant/PLICA/lib/commons-httpclient-3.1.jar" | sudo tee -a /home/vagrant/spark/conf/spark-defaults.conf

#
############################ACTUALIZADO 2.1.1 --> 2.6.0!################################################################################

# Kafka install and setup
#
echo "" | tee -a $LOG_FILE
echo "" | tee -a $LOG_FILE
echo "Downloading and installing Kafka version 2.6.0 for Scala 2.13 ..." | tee -a $LOG_FILE
#curl -Lko /tmp/kafka_2.11-2.1.1.tgz https://www-us.apache.org/dist/kafka/2.1.1/kafka_2.11-2.1.1.tgz
curl -Lko /tmp/kafka_2.13-2.6.0.tgz http://apache.uvigo.es/kafka/2.6.0/kafka_2.13-2.6.0.tgz

mkdir -p /home/vagrant/kafka
cd /home/vagrant/
tar -xvzf /tmp/kafka_2.13-2.6.0.tgz -C kafka --strip-components=1 && rm -f /tmp/kafka_2.13-2.6.0.tgz
